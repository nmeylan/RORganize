class CommentDecorator < ApplicationDecorator
  ...
  def edit_link
    if user_allowed_to_edit?
      h.link_to h.glyph(h.t(:link_edit), 'pencil'), h.edit_comment_path(model), {class: 'edit-comment btn btn-default', remote: true, data: {action: "edit"}}
    end
  end
  ...
  def delete_link
    if User.current.allowed_to?('destroy_comment_not_owner', 'comments', model.project) || model.author?(User.current)
      h.link_to h.glyph(h.t(:link_delete), 'trashcan'), h.comment_path(model),
                {class: 'btn btn-primary danger', method: :delete, remote: true, data: {action: "delete", confirm: h.t(:text_delete_item)}}
    end
  end
