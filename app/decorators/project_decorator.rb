class ProjectDecorator < ApplicationDecorator
  ...
  def display_version_overview
    ...
    condition = %Q(`versions`.`id` IN (#{current_versions.collect(&:id).join(',')})) if versions.to_a.any?
  ...
  def version_overview_init_nil_structure(structure, versions)
    versions.each do |version|
      structure[version.id] = {percent: 100, closed_issues_count: 0, opened_issues_count: 0} unless structure.keys.include?(version.id)
    end
  end
