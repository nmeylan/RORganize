...
module NotificationsHelper
      ...
      safe_concat content_tag :ul, class: 'fancy-list fancy-list-mini', &Proc.new {
        notifications.collect do |notification|
          content_tag :li, class: 'fancy-list-item' do
            ...
            safe_concat content_tag :span, {class: 'right-content-list'}, &Proc.new {
  ...
  def sidebar(filters, projects)
    ...
    content_tag :div, class: 'left-sidebar' do
      safe_concat content_tag :ul, {class: 'filter-sidebar'}, &Proc.new {
        filters.keys.collect do |filter|
          content_tag :li do
            link_to notifications_path(filter: filter),
                    {class: "filter-item #{filter.to_s.eql?(@sessions[:filter_recipient_type]) ? 'selected' : ''}"} do
      ...
      safe_concat content_tag :ul, {class: 'filter-sidebar small'}, &Proc.new {
        projects.keys.collect do |project|
          content_tag :li do
            link_to notifications_path(filter: @sessions[:filter_recipient_type], project: projects[project][:id]),
                    {class: "filter-item #{projects[project][:id].to_s.eql?(@sessions[:filter_project]) ? 'selected' : ''}"} do
