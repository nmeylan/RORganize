...
module QueriesHelper
  ...
  def query_list_header(collection)
    content_tag :tr, class: 'header' do
      list_th sortable('queries.object_type', Query.human_attribute_name(:type), collection.sortable_action), class: 'list-left'
      list_th sortable('queries.name', Query.human_attribute_name(:name), collection.sortable_action), class: 'list-left'
      list_th sortable('users.name', Query.human_attribute_name(:author), collection.sortable_action), class: 'list-left'
      list_th nil
    end
  end
end