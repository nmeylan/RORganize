function apprise(a, b, c) {
    var d = {confirm: false, verify: false, input: false, animate: false, textOk: "Ok", textCancel: "Cancel", textYes: "Yes", textNo: "No"};
    if (b) {
        for (var e in d) {
    ...
    if (b) {
        if (b["animate"]) {
            ...
            jQuery(".appriseOuter").css("top", "-200px").show().animate({top: "100px"}, h)
        } else {
    ...
    if (b) {
        if (b["input"]) {
            if (typeof b["input"] == "string") {
                jQuery(".appriseInner").append('<div class="aInput"><input type="text" class="aTextbox" t="aTextbox" value="' + b["input"] + '" /></div>')
            } else {
    ...
    if (b) {
        if (b["confirm"] || b["input"]) {
            jQuery(".aButtons").append('<button value="ok">' + b["textOk"] + "</button>");
            jQuery(".aButtons").append('<button value="cancel">' + b["textCancel"] + "</button>")
        } else if (b["verify"]) {
            jQuery(".aButtons").append('<button value="ok">' + b["textYes"] + "</button>");
            jQuery(".aButtons").append('<button value="cancel">' + b["textNo"] + "</button>")
        } else {
            jQuery(".aButtons").append('<button value="ok">' + b["textOk"] + "</button>")
        }
    } else {
