class RorganizeController < ApplicationController
  ...
  def index
    unless current_user.nil?
      ...
      @members= current_user.members.includes(:role, :project => [:issues, :attachments]).scoped.select{|member| member.is_project_starred}
      ...
      .includes(:tracker,:version,:status, :project)
      .order(order)
