  ...
  def index
    respond_to do |format|
      if current_user.nil?
        ...
        overview_object_assigned = IssueOverviewHash.new({assigned_to: Issue.group_opened_by_project("assigned_to_id = #{User.current.id}")}, Issue.where(assigned_to: User.current).count)
        overview_object_submitted = IssueOverviewHash.new({assigned_to: Issue.group_opened_by_project("author_id = #{User.current.id}")}, Issue.where(author: User.current).count)
